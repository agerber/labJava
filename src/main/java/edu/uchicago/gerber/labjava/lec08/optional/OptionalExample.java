...
//        System.out.println(rectangle.toString());
        ...
        // Using ifPresent() to provide a lambda expression to execute if a value is present
        ...
        String valueOrDefault = optionalNullable.orElse("Default Value from orElse");
        System.out.println(valueOrDefault);

        // Using orElseGet() to retrieve the value or get it from a supplier functional interface
        String valueOrGetDefault = optionalNullable.orElseGet(() -> "Default Value from Supplier");
        System.out.println(valueOrGetDefault);

        // Using orElseThrow() to throw an exception if the Optional is empty
        try {
            String valueOrThrow = optionalNullable.orElseThrow(() -> new IllegalArgumentException("No value present!"));
